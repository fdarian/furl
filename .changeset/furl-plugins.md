...
- **`order` grammar** — a `config.json` `order` array of `<id>` / `*` / `default:<name>` / `default:*` tokens controls resolver precedence per URL; unset, it defaults to `['*', 'default:raw', 'default:direct', 'default:md-suffix']` (matching plugins, then furl's three keyless probes).
- **`default:` built-ins** — the existing fetch chain (`raw`, `direct`, `md-suffix`, `jina`, `exa`, `firecrawl`) is addressable in `order` as `default:raw`, `default:direct`, etc., or as a group via `default:*`. `jina`/`exa`/`firecrawl` hit third-party APIs, so they're opt-in: add `default:jina` (or `default:*` for all six) to `order`, or pass `--plugin jina` for one call.
